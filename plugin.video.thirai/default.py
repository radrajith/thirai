...
def Main_Categories():
   ...
   path = re.compile('<a href="(.+)"><b>TV Show</b>').findall(link)
   if 'www' not in path:
      path[0] = url + path[0]
   print "Tv Show=" + path[0]
   Add_Dir( '[B]TV Show[/B]', path[0], 20, '')

   path = re.compile('<a href="(.+)"><b>TV Serial</b>').findall(link)
   if 'www' not in path:
      path[0] = url + path[0]
   print "Tv Serial=" + path[0]
   Add_Dir( '[B]TV Serial[/B]', path[0], 20, '')
